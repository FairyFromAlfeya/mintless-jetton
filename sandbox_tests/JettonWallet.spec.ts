...
import { randomAddress, differentAddress, getRandomInt } from "./utils";
...
describe("JettonWallet", () => {
  ...
  beforeAll(async () => {
    ...
    estimateTransferFwd = (jetton_amount, fwd_amount, fwd_payload, custom_payload, prices) => {
      ...
      expect(reverse).toBeGreaterThanOrEqual(Number(feesRes.total));
      return reverse;
    };
  ...
  describe("Content tests", () => {
    ...
    it("minter admin can change content", async () => {
      ...
      loadContent(await jettonMinter.getContent());
      expect(oldContent.get(await sha256("uri"))! === defaultContent.uri).toBe(true);
      //expect(oldContent.get(await sha256('decimals'))! === "6").toBe(true);
    });
  ...
  it("storage stats", async () => {
    ...
    expect(walletStats.cells).toBeGreaterThanOrEqual(Number(actualStats.cells));
    expect(walletStats.bits).toBeGreaterThanOrEqual(Number(actualStats.bits));
    ...
    storageGeneric(res.transactions[1]);
  ...
  it("wallet owner should be able to send jettons", async () => {
    ...
    expect(balanceAfter).toBeGreaterThanOrEqual(Number(balanceBefore));
  ...
  describe("Malformed transfer", () => {
    ...
    it("malformed custom payload", async () => {
      ...
      let errCodes = [9, Errors.invalid_message, Errors.unknown_custom_payload];
      ...
      testPayload = beginCell().storeBuilder(msgTemplate).storeBit(false).storeRef(customPayload).storeBuilder(forwardTail).endCell();
      ...
      testPayload = beginCell().storeBuilder(msgTemplate).storeBit(true).storeRef(customPayload).storeBuilder(forwardTail).endCell();
    ...
    it("malformed forward payload", async () => {
      ...
      let errCodes = [9, Errors.invalid_message];
  ...
  it("correctly sends forward_payload", async () => {
    ...
    expect(mockGas).toBeGreaterThanOrEqual(Number(send_gas_fee));
    ...
    expect(mockGas).toBeGreaterThanOrEqual(Number(receive_gas_fee));
  ...
  describe("Transfer dynamic fees", () => {
    ...
    it("forward_payload should impact transfer fees", async () => {
      let jettonAmount = 1n;
      let forwardAmount = 0n;
      let forwardPayload = beginCell().storeUint(0x123456789abcdefn, 128).endCell();
...
*/
  ...
  // Wallet owner should not be able to burn its jettons
  it("wallet owner should be able to burn jettons", async () => {
    ...
    expect(burn_gas_fee).toBeGreaterThanOrEqual(Number(actualSent));
    expect(burn_notification_fee).toBeGreaterThanOrEqual(Number(actualRecv));
  });
  ...
  it("minter should only accept burn messages from jetton wallets", async () => {
    ...
    const burnNotification = (amount: bigint, addr: Address) => {
      return beginCell().storeUint(Op.burn_notification, 32).storeUint(0, 64).storeCoins(amount).storeAddress(addr).storeAddress(deployer.address).endCell();
    };
  ...
  it("report correct discovery address", async () => {
    ...
    expect(discoveryResult.transactions).toHaveTransaction({
      from: jettonMinter.address,
      to: deployer.address,
      body: beginCell().storeUint(Op.take_wallet_address, 32).storeUint(0, 64).storeAddress(notDeployerJettonWallet.address).storeUint(0, 1).endCell(),
    });
  });
  ...
  it.skip("Minimal discovery fee", async () => {
    ...
    let discoveryResult = await jettonMinter.sendDiscovery(deployer.getSender(), notDeployer.address, false, minimalFee);
     ...
     */
    discoveryResult = await jettonMinter.sendDiscovery(deployer.getSender(), notDeployer.address, false, minimalFee + 1n);
  ...
  describe("Remove governance", () => {
    ...
    it("minter should not be able to force burn tokens", async () => {
      ...
      expect(balanceBefore).toBeGreaterThan(Number(0n));
  ...
  describe("Upgrade", () => {
    ...
    beforeAll(() => {
      ...
      getContractData = async (address: Address) => {
        ...
        if (smc.account.account.storage.state.type != "active") throw "Attempting to get data on inactive account";
      ...
      getContractCode = async (address: Address) => {
        const smc = await blockchain.getContract(address);
        if (!smc.account.account) throw "Account not found";
        if (smc.account.account.storage.state.type != "active") throw "Attempting to get code on inactive account";
    ...
    it("not admin should not be able to upgrade minter", async () => {
      const codeCell = beginCell()
        .storeUint(getRandomInt(1000, (1 << 0) - 1), 32)
        .endCell();
  ...
  it.skip("owner can withdraw excesses", async () => {
    ...
    expect(finalBalance - initialBalance).toBeGreaterThan(Number(toNano("0.99")));
  });
  // implementation detail
  it.skip("not owner can not withdraw excesses", async () => {
    ...
    let finalBalance = (await blockchain.getContract(deployer.address)).balance;
    let finalWalletBalance = (await blockchain.getContract(deployerJettonWallet.address)).balance;
    expect(finalWalletBalance).toBeGreaterThan(Number(toNano("1")));
    expect(finalBalance - initialBalance).toBeLessThan(Number(toNano("0.1")));
  });
  // implementation detail
