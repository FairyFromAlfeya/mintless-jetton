import { Address, beginCell, Cell, Dictionary, DictionaryValue, fromNano, storeMessage, toNano } from "@ton/core";
...
import { buff2bigint, getRandomInt, getRandomTon, testJettonInternalTransfer } from "./utils";
import { Errors, Op } from "../wrappers/JettonConstants";
import { readFile } from "fs/promises";
import { join } from "path";
import { calcStorageFee, collectCellStats, computedGeneric, computeFwdFeesVerbose, getMsgPrices, getStoragePrices, StorageStats } from "../gasUtils";
...
describe("Claim tests", () => {
  ...
  const minimalTransfer = toNano("0.034989413");
  ...
  beforeAll(async () => {
    ...
    getContractData = async (address: Address) => {
      ...
      if (smc.account.account.storage.state.type != "active") throw new Error("Attempting to get data on inactive account");
  ...
  it("should claim and transfer", async () => {
    ...
    const res = await testJetton.sendTransfer(testReceiver.getSender(), toNano("1"), transferAmount, deployer.address, testReceiver.address, claimPayload, 1n);
  ...
  it("should not allow to claim twice", async () => {
    ...
    const res = await testJetton.sendTransfer(testReceiver.getSender(), toNano("1"), totalBalance, deployer.address, deployer.address, claimPayload, 1n);
  ...
  it("should accept only known custom payload", async () => {
    ...
    for (let testOp of ops) {
      ...
      const res = await testJetton.sendTransfer(testReceiver.getSender(), toNano("1"), 1n, deployer.address, deployer.address, claimPayload, 1n);
      expect(res.transactions).toHaveTransaction({
        on: testJetton.address,
        from: testReceiver.address,
        aborted: true,
  ...
  it("claim fee should be accounted for in transfer", async () => {
    ...
    let res = await testJetton.sendTransfer(testReceiver.getSender(), minimalTransfer, 1n, deployer.address, deployer.address, claimPayload, 1n);
    ...
    res = await testJetton.sendTransfer(testReceiver.getSender(), toNano("0.12"), 1n, deployer.address, deployer.address, claimPayload, 1n);
  ...
  it("claim fee should stay same regardless of jetton balance", async () => {
    ...
    const smc = await blockchain.getContract(testJetton.address);
    expect(smc.balance).toBeGreaterThanOrEqual(Number(toNano("10")));
    const res = await testJetton.sendTransfer(testReceiver.getSender(), toNano("0.12"), 1n, deployer.address, deployer.address, claimPayload, 1n);
  ...
  describe("Proofs", () => {
    it("should reject proof from different root", async () => {
      ...
      const res = await evilJetton.sendTransfer(evilDude.getSender(), toNano("1"), 1n, testReceiver.address, evilDude.address, claimPayload, 1n);
      expect(res.transactions).toHaveTransaction({
    ...
    // This one is skipped by default, because it requires @ton/core modifications to ignore hash check at cell creation level
    it.skip("should reject fake proof", async () => {
      ...
      const fakeProof = beginCell().storeUint(3, 8).storeBuffer(airdropCell.hash(0)).storeUint(prunedPath.depth(0), 16).storeRef(prunedPath).endCell({ exotic: true });
      ...
      await expect(evilJetton.sendTransfer(evilDude.getSender(), toNano("1"), 1n, testReceiver.address, evilDude.address, claimPayload, 1n)).rejects.toThrowError(/Hash mismatch/);
    });
    ...
    it("should reject library", async () => {
      ...
      const fakeProof = beginCell().storeUint(2, 8).storeBuffer(airdropCell.hash(0)).storeRef(prunedPath).endCell({ exotic: true });
    ...
    it("should reject pruned branch", async () => {
      ...
      await expect(evilJetton.sendTransfer(evilDude.getSender(), toNano("1"), 1n, testReceiver.address, evilDude.address, claimPayload, 1n)).rejects.toThrow(EmulationError);
    });
    ...
    it("should reject proof for non-present address", async () => {
      ...
      // So someone attempted to replay proof as is

      let res = await evilJetton.sendTransfer(evilDude.getSender(), toNano("1"), 1n, testReceiver.address, evilDude.address, claimPayload, 1n);
      expect(res.transactions).toHaveTransaction({
        on: evilJetton.address,
        from: evilDude.address,
        op: Op.transfer,
      ...
      res = await evilJetton.sendTransfer(evilDude.getSender(), toNano("1"), 1n, testReceiver.address, evilDude.address, claimPayload, 1n);
