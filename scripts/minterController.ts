...
const updateData = async (oldData: Cell, ui: UIProvider) => {
  ...
  do {
    ...
    if (newConfig.transfer_admin !== null) {
      if (!(await promptBool(`Currently admin rights can be transferred to:${curConfig.transfer_admin}\nPreserve?`, ["Yes", "No"], ui))) {
...
const upgradeAction = async (provider: NetworkProvider, ui: UIProvider) => {
  ...
  if (upgradeCode || upgradeData) {
    ...
    const gotTrans = await waitForTransaction(provider, jettonMinterContract.address, contractState.last!.lt.toString(), 10);
    if (gotTrans) {
...
const updateMerkleRoot = async (provider: NetworkProvider, ui: UIProvider) => {
  ...
  curConfig.merkle_root = await promptBigInt("Enter new merkle root in hex or numeric form:", ui);
