...
export async function run(provider: NetworkProvider) {
  ...
  // now let's send 1 TON to first wallet of first package
  ...
  // now let's create jetton wallet owned by wallet
  ...
  await jettonWallet.sendTransfer(walletSender, toNano("0.3"), toNano("0.1"), wallet.address, wallet.address, claimPayload, 1n);
}
